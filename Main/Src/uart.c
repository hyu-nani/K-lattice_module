...
void Clear_Q(UART_DIR Q) {
    switch (Q) {
        case UART1_SEND:
            ...
            UART1Send.TAIL = (u32)((UART1Send.TAIL + 1U) % MAXQ);
            break;

        case UART1_RECV:

            UART1Recv.CNT += (UART1Recv.CNT < MAXQ) ? 1U : 0U;
            UART1Recv.TAIL = (u32)((UART1Recv.TAIL + 1U) % MAXQ);
            break;

        case UART2_SEND:

            UART2Send.CNT += (UART2Send.CNT < MAXQ) ? 1U : 0U;
            UART2Send.TAIL = (u32)((UART2Send.TAIL + 1U) % MAXQ);
            break;

        case UART2_RECV:

            UART2Recv.CNT += (UART2Recv.CNT < MAXQ) ? 1U : 0U;
            UART2Recv.TAIL = (u32)((UART2Recv.TAIL + 1U) % MAXQ);
            break;
